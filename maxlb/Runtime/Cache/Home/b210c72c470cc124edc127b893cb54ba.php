...
<script type="text/javascript">
	$(document).ready(function(){
		...
		if ('<?php echo ($fullusers); ?>'/12 > Math.round('<?php echo ($fullusers); ?>'/12)) {
			var end = Math.round('<?php echo ($fullusers); ?>'/12)+1;
		}else{
			var end = Math.round('<?php echo ($fullusers); ?>'/12);
		}
	...
	function upd_pwd_btn(id){
		...
		$('.modal-body').html(
			"<form method='post' action='http://hxtbaby.com/index.php/Home/Admin/upd_pwd/id/"+id+"'><input type='text' name='password' class='form-control' placeholder='新密码'></input><br/><input type='submit' class='btn btn-primary btn-block' value='更新'></input></form>"
		);
	}
	...
	function upd_type_btn(id){
		...
		$('.modal-body').html(
			"<a href='http://hxtbaby.com/index.php/Home/Admin/upd_type/id/"+id+"' class='btn btn-md btn-warning' style='width:48%;'>确定</a><button data-dismiss='modal' class='btn btn-md btn-primary' style='width:48%;margin-left:4%'>取消</button> "
			);
	}
	...
	function upd_timeout_btn(id){
		...
		$('.modal-body').html(
			"<form method='post' action='http://hxtbaby.com/index.php/Home/Admin/upd_timeout/id/"+id+"'><select class='form-control' name='timeout'><option value='1'>一天</option><option value='30'>一月</option><option value='180'>一季</option><option value='365'>一年</option></select><br/><input type='submit' class='btn btn-primary btn-block' value='更新'></input></form>"
		);
	}
